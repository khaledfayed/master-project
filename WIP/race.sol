...
contract HonestDice {
	...
	function serverSeed(address user, bytes32 seed) public{
		...
		assert(rolls[user].serverSeed != 0); //assertion that server seed is not zero
	}
	//
	...
	function claim(bytes memory secret) public{
		...
		assert(rolls[msg.sender].blocknum == 0); //assert that the user entery has been removed
	}
	...
	function setFeed(address newFeed) public{
		...
		feed = newFeed;
		assert(feed == newFeed); //assert that new feed was assigned
	}
	//
	function lockBetsForWithdraw() public{
		...
		assert(betsLocked == blockNumber); // assert that block number is not zero
	}
	
	function unlockBets() public{
		require(msg.sender == owner);
		betsLocked = 0;
		assert(betsLocked == 0); // assert that block number is zero
	}
